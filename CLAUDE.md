...
- 저장/읽기 규칙은 시스템 메모리 가이드와 동일하게 적용한다.
...
├── plan/
│   ├── request.template.md            # 요청 작성 양식 (사람 작성)
│   ├── work.template.md               # 구현 계획 양식 (Claude 작성)
│   ├── request/<domain>/             # 도메인별 요청 파일
│   └── work/<domain>/                # 도메인별 구현 계획
├── specs/
│   ├── request.schema.json           # Request 유효성 스키마
│   ├── validate-request.js           # request 파일 검증 스크립트
│   └── validate-work.js              # work 파일 독립 검증 스크립트 (PostToolUse 훅 자동 실행)
├── triggers/
│   └── on-work-written.sh            # work Write 시 validate-work.js 실행
├── checklists/work-review.md         # 검토 체크리스트
...
```
Claude가 work.md 저장
  → PostToolUse 훅 → validate-work.js 자동 실행 (독립 검증)
  → 실패 시 Claude가 즉시 수정 후 재저장

git commit
  → Husky pre-commit → npm test 전체 실행
  → 실패 시 커밋 차단

git push
  → GitHub Actions (.github/workflows/test.yml) → npm test 전체 실행
  → Husky 우회(--no-verify) 시에도 원격에서 차단
```

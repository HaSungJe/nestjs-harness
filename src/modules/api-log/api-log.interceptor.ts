...
export class ApiLogInterceptor implements NestInterceptor {
    ...
    intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
        ...
        const saveLog = () => {
            ...
            const entity = this.buildLogEntity(req, res, capturedBody, startTime);
     ...
     */
    private buildLogEntity(req: Request, res: Response, capturedBody: any, startTime: number): ApiLogEntity {
        const entity = new ApiLogEntity();
        entity.method = req.method;
        entity.path = req.originalUrl?.split('?')[0] ?? req.url ?? '';
